...
A_=gen_A(num_classes=80,t=0.4, adj_file='/Users/lizan/Desktop/data/coco/coco_adj.pkl')
...
class GCNResnet(nn.Module):
    ...
    def forward(self, feature, inp, A):
        ...
        adj = gen_adj(Parameter(A.float())).detach()
        x = self.gc1(inp, adj)
        x = self.relu(x)
        x = self.gc2(x, adj)
...
def get_val_result():
    ...
    print('val starting')
    val_dataset = COCO2014('/Users/lizan/Desktop/data/coco', phase='val', inp_name='data/coco/coco_glove_word2vec.pkl')
    ...
    for i, (input, target) in enumerate(val_loader):
       ...
       # pred1=model(input[0],input[2],torch.mul(A_,torch.FloatTensor(80, 80).uniform_(1, 1)))
        pred1=model(input[0],input[2],torch.FloatTensor(80, 80).uniform_(1, 1))
        pred2=model(input[0],input[2],A_)
        pred_list1.append(pred1.data.numpy())
        pred_list2.append(pred1.data.numpy())
    pred_list1 = np.concatenate(pred_list1, axis=0)
    pred_list2 = np.concatenate(pred_list2, axis=0)
    use_gpu = torch.cuda.is_available()
    
    print("get the result'{}".format(np.sum(pred_list1-pred_list2)))
...
def compute_saliency_maps(input, x, model):
    
    model.eval()
    
    x=Variable(x,requires_grad=True)
    saliency = None
    pred=model(input[0],input[2],x)
...
for i, (input, target) in enumerate(val_loader):
    ...
    compute_saliency_maps(input, A_, model)
    if i >2:
        break 
